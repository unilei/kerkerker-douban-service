...
func (h *MoviesHandler) GetMovies(c *gin.Context) {
	...
	if snapshotStore != nil {
		...
		if err := snapshotStore.Load(ctx, moviesCacheKey, &snapData); err == nil {
			snapData = h.doubanService.SyncCategoryDataImages(ctx, snapData)
			if err := h.cache.Set(ctx, moviesCacheKey, snapData); err != nil {
				log.Warn().Err(err).Str("key", moviesCacheKey).Msg("Failed to backfill Redis from Mongo snapshot")
			}
	...
	if snapshotStore != nil {
		if err := snapshotStore.Store(ctx, moviesCacheKey, results); err != nil {
			log.Warn().Err(err).Str("key", moviesCacheKey).Msg("Failed to persist movies snapshot")
		}
	}
