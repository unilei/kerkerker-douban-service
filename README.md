...
插件返回的 TMDB 海报、背景图、详情照片、推荐图和日历图片都进入同一套 R2 镜像器：已有原图映射立即复用，未命中的图片在后台同步，首次响应保留来源地址作为降级路径。TMDB 日历会读取季度详情中的完整集列表并按请求日期范围过滤，缺少季度元数据时才回退到剧集详情的 `last_episode_to_air` / `next_episode_to_air`，不使用剧集的历史 `first_air_date` 冒充单集播出日。
